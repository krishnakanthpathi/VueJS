...
const displayModalDetails =  async (trainId) => {
    ...
    if (trainRow) {
        modalBody.innerHTML = createTrainInfoCard(trainId);
...
const displayTrainDetails = async (trainId) => {
    ...
    const trainRow = await getTrainRowById(trainId);
    if (trainRow) {
        trainDetailsDiv.innerHTML = await createTrainInfoCard(trainId);
...
const TrainInfoForm = document.getElementById('train-info-form');

TrainInfoForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const trainId = e.target.trainId.value;
    await displayTrainDetails(trainId);
});
